...
12. **LEADERBOARD** — `localStorage`-backed, no backend. `loadLB`/`saveLB` read/write JSON arrays under `lb_scores` and `lb_times`; `recordRun()` is called on player death. The lifetime enemy-kill counter (`#hKills`, bottom-right HUD) lives right next to this — `loadEnemyKillCount`/`saveEnemyKillCount` persist a single number under `vd_enemy_kill_count`, incremented in `onEnemyDeath()` whenever `playerKill` is true (same condition that awards score).
